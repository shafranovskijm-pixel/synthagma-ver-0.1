...
|---|---|---|
| Локальный PostgreSQL dry-run миграции и RLS | PASS на PostgreSQL 17.11 (`127.0.0.1:55443`) воспроизводимым runner на объединённом SHA `fa5e29d06`: обязательный `ROLLBACK`, применение миграции только к одноразовой БД, admin CRUD/архив, teacher read-only, learner visibility, запрет незачисленного и cross-tenant доступа, запрет cross-course/module связей, неизменность старого курса и unpublished-состояние нового | Это изолированный vanilla PostgreSQL со стендовыми ролями/функциями, а не Supabase staging. Доказательство: `D:\CodexTmp\course-library-local-acceptance\20260903-082844-0e4d4688\result.json`; listener после прогона остановлен; JWT Supabase, PostgREST, Storage API и signed URL не проверены |
| Целевые автоматические тесты | Текущий библиотечный UI/API/RLS-набор: 7 файлов, 61/61, exit 0; отдельный merge-proof ГОРЭЛТЕХ `v14` + библиотека: 6 файлов, 63/63, exit 0 | Это unit/component/API/security-contract тесты, не DB-backed browser E2E; в старых hook-тестах остаются известные React `act` и неполные Supabase mock warnings |
| Полный Vitest-прогон | 214 файлов, 1 628/1 628 тестов, exit 0 | Прогон выполнен после слияния текущего production `main` с ГОРЭЛТЕХ `v14`, расширения PostgreSQL-контракта и добавления UI-сценариев библиотеки |
...
После повторной проверки подтверждены семь рабочих замен первичными источниками:
...
3. Пять критических source/playback-кандидатов требуют устранения разрывов; семь подтверждённых замен ещё не утверждены, поэтому все 28 внешних строк остаются `import_action=HOLD`.
