-- Phase 4B.1.a — server-side aggregates for organization dashboard.
-- Additive migration. No existing objects are altered or dropped.
...
BEGIN
  ...
  enr AS (
    SELECT e.user_id, e.progress, e.completed_at
  ...
  SELECT
    ...
    (SELECT count(DISTINCT user_id)::bigint FROM enr WHERE completed_at IS NOT NULL),
