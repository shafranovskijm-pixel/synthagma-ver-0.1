...
BEGIN
  ...
  -- Валидация входного массива ДО любых изменений: пустой пакет не должен
  -- сбрасывать is_current у предыдущей партии.
  ...
  IF v_docs_count = 0 THEN
    RAISE EXCEPTION 'p_docs must contain at least one document';
  END IF;
  IF v_docs_count > 500 THEN
    RAISE EXCEPTION 'p_docs exceeds max batch size (500), got %', v_docs_count;
  END IF;
  ...
  -- Сериализация параллельных пакетов одной группы: ОДИН 64-битный ключ
  -- (pg_advisory_xact_lock(bigint)), блокировка ДО чтения MAX(package_version).
  PERFORM pg_advisory_xact_lock(
    hashtextextended(p_organization_id::text || ':' || p_group_id::text, 0)
  );
