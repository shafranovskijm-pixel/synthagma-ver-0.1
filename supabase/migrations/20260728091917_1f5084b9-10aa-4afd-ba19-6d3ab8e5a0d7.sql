-- =========================================================
-- Phase 3: Organization students pagination fixes + new counters
-- =========================================================

-- 1. Redefine get_organization_students_page with proper doc types and status filter
...
BEGIN
  ...
  joined AS (
    SELECT b.*,
           ...
           COALESCE(be.has_active, false)                                           AS r_has_active,
           COALESCE(be.has_completed, false)                                        AS r_has_completed,
  ...
  filtered AS (
    ...
    AND (
      ...
      OR (p_status = 'active'       AND j.r_has_active)
      OR (p_status = 'completed'    AND j.r_has_completed)
...
-- 2. Organization students counts (active / archived / total) — filters-independent
CREATE OR REPLACE FUNCTION public.get_organization_students_counts(
  p_organization_id uuid
)
RETURNS TABLE(active_count bigint, archived_count bigint, total_count bigint)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = public
...
REVOKE ALL ON FUNCTION public.get_organization_students_counts(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_organization_students_counts(uuid) TO authenticated, service_role;

-- 3. Organization student group counts
CREATE OR REPLACE FUNCTION public.get_organization_student_group_counts(
  p_organization_id uuid
)
RETURNS TABLE(
  group_id uuid,
...
BEGIN
  ...
  SELECT
    p.student_group_id                                        AS group_id,
    COUNT(*)                                          ::bigint AS total_count,
    COUNT(*) FILTER (WHERE p.archived_at IS NULL)     ::bigint AS active_count,
    COUNT(*) FILTER (WHERE p.archived_at IS NOT NULL) ::bigint AS archived_count
  FROM public.profiles p
...
-- 4. Decrypted passwords for a list of student user_ids (on-demand only)
...
BEGIN
  ...
  v_is_staff := public.has_org_staff_permission(p_organization_id, 'students.write');

  IF NOT (v_is_admin OR v_is_owner OR v_is_staff) THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;

  v_key := public._get_pw_key();

  RETURN QUERY
  SELECT p.user_id,
         CASE
           WHEN p.generated_password_encrypted IS NULL THEN NULL
           ELSE pgp_sym_decrypt(p.generated_password_encrypted, v_key)
         END AS decrypted_password
  FROM public.profiles p
  WHERE p.organization_id = p_organization_id
    AND p.user_id = ANY(p_user_ids)
    AND public.is_student_profile(p.user_id);
END;
$fn$;
