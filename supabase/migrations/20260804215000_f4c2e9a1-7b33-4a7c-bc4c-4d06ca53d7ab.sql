-- Идемпотентная выдача документов об образовании для ФИС ФРДО.
-- Повторный экспорт того же original-документа по точному зачислению
-- возвращает ранее выданные номера и не расходует последовательности повторно.
...
BEGIN
  ...
  IF p_course_id IS NULL THEN
    RAISE EXCEPTION 'exact course_id is required';
  END IF;
  ...
  IF v_items_count = 0 THEN
    RAISE EXCEPTION 'items must be a non-empty array';
  END IF;
  IF v_items_count > 500 THEN
    RAISE EXCEPTION 'items exceed max batch size (500)';
  END IF;
  ...
  IF p_group_id IS NOT NULL THEN
    SELECT g.course_id
      INTO v_group_course_id
      FROM public.student_groups g
     WHERE g.id = p_group_id
       AND g.organization_id = p_organization_id;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'group % does not belong to organization', p_group_id;
    END IF;
  ...
  -- Совместимый с предыдущей версией ключ: все выдачи организации сериализованы.
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_enrollment_id IS NULL THEN
      RAISE EXCEPTION 'enrollment_id is required for every item';
    END IF;
    IF v_doc_type IS NULL THEN
      RAISE EXCEPTION 'document_type is required for every item';
    END IF;
    IF NULLIF(v_item->>'issue_date', '') IS NULL THEN
      RAISE EXCEPTION 'issue_date is required for every item';
    END IF;
    ...
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'enrollment % does not belong to student %/exact course %',
        v_enrollment_id, v_user_id, p_course_id;
    END IF;

    -- Идемпотентность: повторная выгрузка не выпускает второй original.
    SELECT r.id
      INTO v_existing_id
      FROM public.education_document_records r
     WHERE r.organization_id = p_organization_id
       AND r.enrollment_id = v_enrollment_id
       AND lower(btrim(r.document_type)) = lower(btrim(v_doc_type))
       AND lower(btrim(r.document_status)) IN ('original', 'оригинал')
  ...
  RETURN QUERY
    SELECT *
     ...
     ORDER BY array_position(v_ids, r.id);
END;
$$;
