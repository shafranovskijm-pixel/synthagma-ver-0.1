...
-- This ensures all login credentials work correctly

-- student aaaassaaaaaaaaaaaaa
UPDATE auth.users SET encrypted_password = crypt('asssssssssssfd52352', gen_salt('bf'))
WHERE id = '9759e3c5-05b8-4441-b3ce-13058cfeab0a';
...
-- student_23330 (already synced but including for completeness)
UPDATE auth.users SET encrypted_password = crypt('nw9PiFUjuu', gen_salt('bf'))
...
-- student_54209
UPDATE auth.users SET encrypted_password = crypt('v2tTZd2dCs', gen_salt('bf'))
WHERE id = 'c09adbaa-1dca-4df9-a5e2-a71891a5dae8';
...
-- student_78998
UPDATE auth.users SET encrypted_password = crypt('fpj2hff9', gen_salt('bf'))
WHERE id = 'ad84a1d6-d300-48aa-8594-6b9630a32071';
...
-- student_87317
UPDATE auth.users SET encrypted_password = crypt('RtZlKOmqE3', gen_salt('bf'))
