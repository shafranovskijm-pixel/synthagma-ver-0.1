...
Отвечай ТОЛЬКО JSON-массивом, без markdown-обертки.`;
      ...
      if (isWorkerProfession && lessonType === "practice") {
        defaultContentPrompt = EDU_PREAMBLE + `Ты эксперт по профессиональному обучению рабочим профессиям. Создай практическое задание (кейс / производственную ситуацию).
Правила:
...
10. Акцент на ПРАКТИЧЕСКИЕ НАВЫКИ: какие инструменты взять, какие материалы подготовить, в какой последовательности выполнять операции, как проверить качество.${contextNote}`;
        ...
        defaultContentPrompt = EDU_PREAMBLE + `Ты эксперт по профессиональному обучению рабочим профессиям (электромонтёр, сварщик, слесарь, штукатур, стропальщик и др.). Создай подробный учебный материал для программы профессионального обучения (300 часов).
Правила:
1. Структурированный текст с заголовками (используй Markdown)
...
14. Используй ## для заголовков разделов и ### для подзаголовков.${contextNote}`;
        ...
        defaultContentPrompt = EDU_PREAMBLE + `Ты эксперт по промышленной безопасности и нормативам Ростехнадзора. Создай практическое задание (кейс / ситуационную задачу).
Правила:
1. Структурированный текст с заголовками (используй Markdown)
...
9. ЗАПРЕЩЕНО использовать LaTeX ($...$, $$...$$). Пиши формулы простым текстом.${contextNote}`;
      ...
      defaultContentPrompt = EDU_PREAMBLE + `Ты эксперт по промышленной безопасности и нормативам Ростехнадзора. Создай подробный учебный материал.
Правила:
1. Структурированный текст с заголовками (используй Markdown)
...
- НЕ начинай с мета-фраз («Отлично!», «Конечно!», «Подготовлю...»).`;
      ...
      const workerQuestionsPrompt = EDU_PREAMBLE + `Ты эксперт по профессиональному обучению рабочим профессиям. Создай тестовые вопросы для проверки ПРАКТИЧЕСКИХ знаний и навыков.
...
Создай 15 вопросов разной сложности. Отвечай ТОЛЬКО JSON-массивом.`;
      ...
      const defaultQuestionsPrompt = EDU_PREAMBLE + `Ты эксперт по промышленной безопасности и нормативам Ростехнадзора. Создай тестовые вопросы.
Отвечай СТРОГО в формате JSON-массива, каждый элемент:
- "question": текст вопроса
- "options": массив из 4 вариантов ответа
- "correctAnswer": индекс правильного ответа (0-3)
...
Отвечай ТОЛЬКО JSON, без markdown-обертки.`;
      ...
      const verifyPrompt = EDU_PREAMBLE + `Ты эксперт-верификатор в области промышленной безопасности, охраны труда и нормативов Ростехнадзора.
