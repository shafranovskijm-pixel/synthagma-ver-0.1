...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `КОНТЕКСТ: Это официальная система профессионального обучения, лицензированная в соответствии с законодательством Российской Федерации. Все темы являются частью образовательных программ повышения квалификации и переподготовки специалистов. Материалы предназначены исключительно для образовательных целей.

Ты — ИИ-помощник образовательной платформы СИНТАГМА. 
