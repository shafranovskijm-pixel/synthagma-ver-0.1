...
Deno.serve(async (req) => {
  ...
  if (roleError || isAdmin !== true) return json({ ok: false, error: "forbidden" }, 403);
  ...
  if (status === "pending") return json({ ok: true, status: "pending" });
  return json({ ok: false, status: "failed", error: "telegram_delivery_failed" }, 502);
});
