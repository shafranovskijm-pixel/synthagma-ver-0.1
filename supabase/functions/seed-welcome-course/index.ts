...
async function seedCourseForOrg(supabase: any, organizationId: string) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { organizationId, seedAll } = body;
    ...
    if (seedAll) {
      ...
      for (const org of orgs || []) {
        try {
          const res = await seedCourseForOrg(supabase, org.id);
    ...
    const result = await seedCourseForOrg(supabase, organizationId);
