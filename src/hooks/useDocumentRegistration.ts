...
const generateRegNumber = (docType: string, date: string, _index: number, typeCounters: Map<string, Map<number, number>>): string => {
...
export function useDocumentRegistration(organizationId: string) {
      ...
      try {
        ...
        for (const doc of issuanceLog || []) {
          ...
          documentRecords.push({
            id: doc.id, original_id: doc.id, reg_number: doc.reg_number, document_type: docType,
            document_name: doc.document_name, direction: "outgoing", date: doc.issued_at,
            related_entity: doc.user_name, related_entity_type: "student",
            notes: doc.send_method ? `Отправлено: ${doc.send_method}` : null,
            source: "issuance_log", is_editable: true, file_url: doc.file_url || null,
          });
        }
        ...
        for (const doc of companyDocs || []) {
          ...
          documentRecords.push({
            id: doc.id, original_id: doc.id, reg_number: doc.contract_number, document_type: docType,
            document_name: doc.name, direction: doc.type === "contract" ? "incoming" : "outgoing",
            date: doc.contract_date || doc.uploaded_at, related_entity: (doc.companies as any)?.name || null,
            related_entity_type: "company",
            notes: doc.amount ? `Сумма: ${doc.amount} ₽` : null,
            source: "company_document", is_editable: true, file_url: doc.file_url || null,
          });
        }
        ...
        for (const entry of enrollmentHistory || []) {
          ...
          if (entry.action === "enrolled") {
            ...
            documentRecords.push({
              id: `enrollment_${entry.id}`, original_id: entry.id,
              reg_number: entry.enrollment_id ? `ПР-${entry.enrollment_id.slice(0, 8).toUpperCase()}` : null,
              document_type: "enrollment_order", document_name: docName, direction: "outgoing",
              date: entry.created_at, related_entity: studentName, related_entity_type: "student",
              notes: null, source: "enrollment", is_editable: false, file_url: fileUrl,
            });
            ...
            const fileUrl = orgDocFileMap.get(docName.toLowerCase()) || null;
            documentRecords.push({
              id: `expulsion_${entry.id}`, original_id: entry.id,
              reg_number: entry.enrollment_id ? `ПР-${entry.enrollment_id.slice(0, 8).toUpperCase()}` : null,
              document_type: entry.action === "completed" ? "certificate" : "expulsion_order",
              document_name: docName, direction: "outgoing", date: entry.created_at,
              related_entity: studentName, related_entity_type: "student",
              notes: null, source: "enrollment", is_editable: false, file_url: fileUrl,
            });
          }
        }
  ...
  const handleViewDocument = async (record: DocumentRecord) => {
    ...
    try {
      if (record.file_url.startsWith("http")) { window.open(record.file_url, "_blank"); }
        ...
        if (signedUrl) {
          const res = await fetch(signedUrl); const html = await res.text();
          const blob = new Blob([html], { type: "text/html" });
          window.open(URL.createObjectURL(blob), "_blank");
        } else { toast.error("Не удалось открыть документ"); }
      }
    } catch (error) { console.error("Error viewing document:", error); toast.error("Ошибка при открытии документа"); }
  };
  ...
  const handleDownloadDocument = async (record: DocumentRecord) => {
    ...
    try {
      ...
      if (!url.startsWith("http")) {
        ...
        if (!signedUrl) { toast.error("Не удалось скачать документ"); return; }
        url = signedUrl;
      }
      await downloadHtmlFile(url, record.document_name);
      toast.success("Документ скачан");
    } catch (error) { console.error("Error downloading document:", error); toast.error("Ошибка при скачивании документа"); }
  };

  const handleEditClick = (record: DocumentRecord) => {
    if (!record.is_editable) { toast.info("Этот документ нельзя редактировать"); return; }
    setEditingRecord(record); setEditRegNumber(record.reg_number || "");
  };
  ...
  const handleSaveRegNumber = async () => {
    ...
    try {
      ...
      setEditingRecord(null); setEditRegNumber("");
    } catch (error) { console.error("Error saving reg number:", error); toast.error("Ошибка при сохранении"); }
  ...
  const handleAddDocument = async () => {
    ...
    try {
      ...
      const { data: insertedDoc, error } = await supabase.from("document_issuance_log").insert({
        organization_id: organizationId, user_id: user.id, user_name: userName,
        document_type: newDocument.document_type, document_name: newDocument.document_name.trim(),
        reg_number: newDocument.reg_number.trim() || null, issued_at: newDocument.date.toISOString(),
        send_method: newDocument.notes.trim() || null,
      }).select().single();
      if (error) throw error;
      const newRecord: DocumentRecord = {
        id: insertedDoc.id, original_id: insertedDoc.id, reg_number: insertedDoc.reg_number,
        document_type: newDocument.document_type, document_name: insertedDoc.document_name,
        direction: newDocument.direction, date: insertedDoc.issued_at, related_entity: userName,
        related_entity_type: "student", notes: insertedDoc.send_method ? `Примечание: ${insertedDoc.send_method}` : null,
        source: "issuance_log", is_editable: true, file_url: null,
      };
    ...
    } catch (error) { console.error("Error adding document:", error); toast.error("Ошибка при добавлении документа"); }
    finally { setSaving(false); }
  };
  ...
  const exportToExcel = async () => {
    ...
    const exportData = filteredRecords.map((record, index) => ({
      "№ п/п": index + 1, "Рег. номер": record.reg_number || "—",
  ...
  return {
    ...
    handleSaveRegNumber, generateSuggestedNumber,
    handleAddDocument, generateNewDocNumber, exportToExcel,
  };
}
