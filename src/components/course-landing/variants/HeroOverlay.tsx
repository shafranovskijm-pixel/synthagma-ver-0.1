...
/**
 * Hero «Overlay» — фон на весь экран, текст слева внизу.
 * Базовый дефолтный вариант.
 */
    ...
    <section className="relative min-h-[480px] flex items-end overflow-hidden">
      <div className="absolute inset-0">
        {bg ? (
          ...
          <div className="w-full h-full" style={{ background: `linear-gradient(135deg, ${accent}22 0%, ${accent}44 100%)` }} />
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-black/10" />
      </div>

      {isEditing && onBackgroundChange && (
        <button onClick={onBackgroundChange} className="absolute top-4 right-4 z-20 bg-white/90 backdrop-blur text-sm px-4 py-2 rounded-lg shadow hover:bg-white transition font-medium">
          Изменить фон секции
        </button>
      )}
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-6 pb-12 pt-32">
        ...
        {isEditing ? (
          <h1 contentEditable suppressContentEditableWarning
            className="landing-heading text-3xl md:text-5xl font-bold text-white mb-4 outline-none border-b-2 border-dashed border-white/30 focus:border-white/60 transition"
            onBlur={(e) => onTitleChange?.(e.currentTarget.textContent || "")}
          >{title}</h1>
        ) : (
          <h1 className="landing-heading text-3xl md:text-5xl font-bold text-white mb-4">{title}</h1>
        )}
        {isEditing ? (
        ...
        <div className="flex flex-wrap items-center gap-4">
          {showPrice && (price > 0 ? (
            ...
            <Badge className={`text-base px-4 py-1 bg-white/20 text-white border-white/30 ${radiusButtonClass[theme.radius]}`}>Бесплатно</Badge>
          ))}
          {enrollButton}
