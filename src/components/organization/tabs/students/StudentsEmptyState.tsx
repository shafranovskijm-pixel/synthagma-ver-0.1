  ...
  return (
    <div className="py-8">
      ...
      <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
        ...
        <EmptyCard title="Документооборот" description="Собирайте документы учеников, формируйте поддерживаемые приказы и протоколы, подготавливайте XLSX для ФИС ФРДО." icon={FileText} color="muted" items={[{ icon: FileSpreadsheet, text: "Excel и XLSX для ФИС ФРДО" }, { icon: FileText, text: "Приказы и протоколы" }, { icon: MessageCircle, text: "Напоминания о документах" }]} action={onNavigateToFRDO && <Button onClick={onNavigateToFRDO} variant="outline" className="w-full rounded-xl gap-2 mt-2"><FileText className="w-4 h-4" />Подготовить данные ФИС ФРДО</Button>} />
      </div>
    </div>
  );
}
