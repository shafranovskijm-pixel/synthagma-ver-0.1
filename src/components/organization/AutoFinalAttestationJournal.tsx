    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { icon: CheckCircle, color: "indigo", label: "Средний балл", value: h.stats.avgScore === null ? "—" : `${h.stats.avgScore}%` },
        ].map((s, i) => (
      ...
      {h.filteredRecords.length > 0 ? (
        ...
        <TableBody>{h.filteredRecords.slice(0, 100).map(r => (
          <TableRow key={r.id}>
            ...
            <TableCell className="text-center">{r.final_test_score !== null && r.final_test_max_score !== null ? <span className={cn("inline-flex items-center px-2 py-1 rounded-lg font-medium text-sm", h.getScoreColor(r.final_test_score, r.final_test_max_score))}>{r.final_test_score}/{r.final_test_max_score}<span className="ml-1 text-xs opacity-70">({Math.round((r.final_test_score / r.final_test_max_score) * 100)}%)</span></span> : <span className="text-muted-foreground text-sm">{r.manual_credited_at ? "Очный зачёт" : "Не сдан"}</span>}</TableCell>
            <TableCell className="text-center">{r.final_test_score === null && !r.final_test_passed ? <div className="flex items-center justify-center gap-1 text-muted-foreground"><Clock className="w-4 h-4" /><span className="text-sm">Ожидается</span></div> : r.final_test_passed ? <div className="flex items-center justify-center gap-1 text-green-600"><CheckCircle className="w-4 h-4" /><span className="text-sm font-medium">{r.manual_credited_at ? "Зачтено организацией" : "ЗАЧЁТ"}</span></div> : <div className="flex items-center justify-center gap-1 text-red-600"><XCircle className="w-4 h-4" /><span className="text-sm font-medium">НЕЗАЧЁТ</span></div>}</TableCell>
