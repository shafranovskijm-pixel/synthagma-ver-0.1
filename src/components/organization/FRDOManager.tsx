...
export function FRDOManager({ organizationId }: FRDOManagerProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      for (const e of enrollmentsData || []) {
        ...
        if (courseData && !courseSet.has(courseData.id)) {
          courseSet.set(courseData.id, {
            id: courseData.id, title: courseData.title,
            frdo_program_type: courseData.frdo_program_type, frdo_document_type: courseData.frdo_document_type,
      ...
      const studentsList: Student[] = studentProfiles.map(p => ({
        user_id: p.user_id, name: p.full_name || "Без имени", email: p.email || "",
  ...
  const getFrdoStatus = (userId: string): { status: FRDOStatus; missingFields: string[] } => {
    ...
    if (!data) return { status: "empty", missingFields: requiredFields.map(f => f.label) };
    const missing: string[] = [];
    for (const field of requiredFields) {
      if (!data[field.key as keyof FRDOData]) missing.push(field.label);
    }
  ...
  const filteredStudents = students.filter(student => {
    if (searchQuery) {
      ...
      if (!student.name.toLowerCase().includes(query) && !student.email.toLowerCase().includes(query)) return false;
    }
    if (statusFilter !== "all") {
    ...
    try {
      ...
      for (const userId of exportUserIds) {
        ...
        const data: FRDOData = frdoData || {
          user_id: userId,
          last_name: student.name.split(" ")[0] || "", first_name: student.name.split(" ")[1] || "",
          middle_name: student.name.split(" ")[2] || "", birth_date: "", gender: "",
          snils: "", citizenship_code: "643", education_level: "",
          education_doc_last_name: "", education_doc_series: "", education_doc_number: "",
          training_form: "Очная", financing_source: "Платное обучение",
          education_form: "в образовательной организации", professional_area: "",
          specialty_group: "", qualification_name: "", profession_name: "", qualification_rank: "",
        };
        ...
        const processEnrollment = (enrollment: EnrollmentData | null) => {
          ...
          // Register document
          supabase.from("education_document_records").insert({
          ...
          if (exportType === "dpo") {
            ...
            rows.push(buildDPORow({
              documentType, docNumber: docNum, regNumber: regNum,
              issueDate: formatDateForFRDO(enrollment?.completed_at || ""),
              programType, programName: enrollment?.course_title || "",
              professionalArea, specialtyGroup, qualificationName,
              educationLevel: data.education_level,
              educationDocLastName: data.education_doc_last_name,
              educationDocSeries: data.education_doc_series,
              educationDocNumber: data.education_doc_number,
  ...
  // Check if any course has PO settings
  ...
  const missingFieldsStats = (() => {
    ...
    for (const field of requiredFields) fieldCounts[field.label] = 0;
    for (const student of students) {
      const { missingFields } = getFrdoStatus(student.user_id);
      for (const field of missingFields) {
        if (fieldCounts[field] !== undefined) fieldCounts[field]++;
      }
    }
    return Object.entries(fieldCounts).filter(([_, count]) => count > 0).sort((a, b) => b[1] - a[1]);
  })();
    ...
    <div className="space-y-6">
        ...
        <div className="bg-card rounded-2xl border border-border p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {missingFieldsStats.map(([field, count]) => (
              <div key={field} className="flex items-center justify-between p-3 rounded-xl bg-amber-500/5 border border-amber-500/20">
                <span className="text-sm font-medium">{field}</span>
                <span className="text-sm font-bold text-amber-600 bg-amber-500/10 px-2 py-0.5 rounded-lg">{count}</span>
              </div>
            ))}
          </div>
          <p className="text-xs text-muted-foreground mt-3">Количество учеников, у которых отсутствует данное поле</p>
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3 flex-1">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input placeholder="Поиск по имени или email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 rounded-xl" />
          </div>
        ...
        {selectedStudents.size > 0 && (
          <div className="flex items-center gap-2">
            <Button onClick={() => handleBulkExport("dpo")} className="rounded-xl gap-2" disabled={isExporting}>
              {isExporting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
              Экспорт ДПО ({selectedStudents.size})
            </Button>
            {hasPOCourses && (
              <Button variant="secondary" onClick={() => handleBulkExport("po")} className="rounded-xl gap-2" disabled={isExporting}>
                {isExporting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  <th className="text-left px-4 py-4 text-sm font-medium text-muted-foreground w-12">
                    <input type="checkbox" checked={selectedStudents.size === filteredStudents.length && filteredStudents.length > 0} onChange={toggleSelectAll} className="w-4 h-4 rounded" />
                  </th>
              ...
              <tbody>
                {filteredStudents.map((student) => {
                  ...
                  return (
                    <tr key={student.user_id} className={`border-b border-border last:border-0 hover:bg-muted/30 transition-colors ${isSelected ? 'bg-primary/5' : ''}`}>
                      <td className="px-4 py-4">
                        <input type="checkbox" checked={isSelected} onChange={() => toggleStudentSelection(student.user_id)} className="w-4 h-4 rounded" />
                      </td>
                      <td className="px-4 py-4">
                        <div>
                      ...
                      <td className="px-4 py-4">
                        {status === "complete" ? (
                          ...
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded flex items-center justify-center bg-amber-500/10" title={`Не заполнено: ${missingFields.join(", ")}`}>
                      ...
                      <td className="px-4 py-4">
                        <Button size="sm" variant="outline" className="rounded-lg gap-1" onClick={() => openStudentExport(student)}>
      ...
      <FRDOExportDialog
        ...
        student={selectedStudentForExport ? {
          id: selectedStudentForExport.user_id, user_id: selectedStudentForExport.user_id,
          name: selectedStudentForExport.name, email: selectedStudentForExport.email,
        } : null}
        organizationId={organizationId}
        enrollment={selectedEnrollmentForExport ? {
          id: selectedEnrollmentForExport.course_id, course_id: selectedEnrollmentForExport.course_id,
          course_title: selectedEnrollmentForExport.course_title, started_at: selectedEnrollmentForExport.started_at,
          completed_at: selectedEnrollmentForExport.completed_at, time_spent: selectedEnrollmentForExport.time_spent,
        } : null}
      />
    </div>
  );
