...
export function ChatTab({ studentUserId, organizationId, currentUserId, studentName }: ChatTabProps) {
  ...
  const loadSignedUrl = async (url: string) => {
    ...
    if (signed) {
      setSignedUrls((prev) => new Map(prev).set(url, signed));
    }
  };
  ...
  const loadMessages = async () => {
    ...
    for (const msg of msgs) {
      if (msg.attachment_url && !msg.attachment_url.startsWith("http")) {
        loadSignedUrl(msg.attachment_url);
      }
    }

    // Mark unread messages as read
    const unread = msgs.filter(m => !m.is_read && m.sender_user_id !== currentUserId);
    if (unread.length > 0) {
      await supabase
        .from("org_student_messages")
        .update({ is_read: true })
      ...
      <div className="flex-1 overflow-y-auto space-y-3 mb-4 pr-1">
        {messages.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>Начните переписку с {studentName}</p>
          </div>
        ) : (
          messages.map((msg) => {
