...
export function CompaniesManager({ organizationId }: CompaniesManagerProps) {
  ...
  const handleViewDocument = async (doc: CompanyDocument) => {
    ...
    // Check if it's a DOC file generated by our system
    const fileExtension = doc.file_path.split('.').pop()?.toLowerCase();
    
    if (fileExtension === 'doc') {
      // Download and display as HTML in preview modal
      setIsLoadingPreview(true);
      setPreviewDocumentName(doc.name);
      ...
      try {
        ...
        // Read the DOC file as text (it's actually HTML)
        const text = await data.text();
        
        // Extract body content from DOC HTML
        ...
        setPreviewDocumentHtml(previewHtml);
      } catch (error) {
      ...
      // For PDFs, open in new tab with signed URL
      try {
        const { data, error } = await supabase.storage
          .from("company-documents")
