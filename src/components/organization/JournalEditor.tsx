...
export function JournalEditor(props: JournalEditorProps) {
  const {
    loading, loadError, saving, writeBlocked, reloadJournal, students, courses, selectedCourse, setSelectedCourse,
  ...
  const mutationDisabled = loading || saving || writeBlocked;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4"><Button variant="ghost" size="icon" aria-label="Закрыть журнал" onClick={props.onClose} className="rounded-xl"><ArrowLeft className="w-5 h-5" /></Button><div><h2 className="text-xl font-semibold">{props.journalTitle}</h2><p className="text-sm text-muted-foreground">Онлайн ведение журнала</p></div></div>
        <Button variant="outline" className="rounded-xl" disabled={mutationDisabled} onClick={() => setShowCreateDialog(true)}><Plus className="w-4 h-4 mr-2" />Новый журнал</Button>
      </div>

      {loadError && <div role="alert" className="space-y-3 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm">
        <p>{loadError}</p>
        <Button type="button" variant="outline" onClick={reloadJournal} disabled={loading || saving}>Перезагрузить журнал</Button>
      </div>}

      {loading && existingJournals.length === 0 ? (
        <div className="flex items-center justify-center h-64" role="status"><SigmaSpinner size="lg" /></div>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          {journalInstance && (
            <>
              ...
              <Button variant="ghost" size="icon" aria-label="Удалить журнал" disabled={mutationDisabled} className="text-destructive hover:text-destructive rounded-xl" onClick={() => setShowDeleteDialog(true)}><Trash2 className="w-4 h-4" /></Button>
            </>
          )}
        </div>
      ) : !loadError && (
        <div className="bg-card rounded-2xl border border-border p-8 text-center"><Users className="w-12 h-12 mx-auto text-muted-foreground mb-4" /><h3 className="font-semibold mb-2">Нет журналов</h3><Button onClick={() => setShowCreateDialog(true)} disabled={mutationDisabled} className="rounded-xl"><Plus className="w-4 h-4 mr-2" />Создать журнал</Button></div>
      )}
          ...
          <TableBody>{students.map(s => (
            <TableRow key={s.user_id}>
              ...
              {dates.map(d => {
                ...
                const attendance = ATTENDANCE_VALUES.find(option => option.value === val);
                const AttendanceIcon = attendance?.icon;
                const status = isAttendanceJournal
                  ? attendance?.label ?? (val ? `Неизвестное значение: ${val}` : "Не отмечено")
                  : val || "Не выставлено";
                return (
                  <TableCell key={d.toISOString()} className="text-center p-1">
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="ghost" size="sm" disabled={mutationDisabled}
                          aria-label={`${s.full_name}, ${format(d, "dd.MM.yyyy")}: ${status}`}
                          className={cn("w-10 h-10 p-0 rounded-lg", isAttendanceJournal ? attendance?.color : (val && "font-semibold"))}>
                          {isAttendanceJournal ? (AttendanceIcon
                            ? <AttendanceIcon className="w-4 h-4" aria-hidden="true" />
                            : <span className="max-w-full truncate text-muted-foreground" title={val || undefined}>{val || "—"}</span>) : (val || "—")}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-2" align="center">
                        <div className={cn("gap-1", isAttendanceJournal ? "flex flex-col" : "grid grid-cols-4")}>
                          {isAttendanceJournal ? ATTENDANCE_VALUES.map(o => <Button key={o.value} variant="ghost" size="sm" disabled={mutationDisabled} className={cn("justify-start gap-2", o.color)} onClick={() => updateEntry(s.user_id, d, o.value)}><o.icon className="w-4 h-4" />{o.label}</Button>) : GRADE_VALUES.map(g => <Button key={g} variant="ghost" size="sm" disabled={mutationDisabled} onClick={() => updateEntry(s.user_id, d, g)}>{g}</Button>)}
                          <Button variant="ghost" size="sm" disabled={mutationDisabled} className="text-muted-foreground justify-start" onClick={() => updateEntry(s.user_id, d, "")}>{isAttendanceJournal ? <><Minus className="w-4 h-4 mr-2" />Очистить</> : "×"}</Button>
                        </div>
                      </PopoverContent>
                    </Popover>
                  </TableCell>
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Создать журнал</DialogTitle></DialogHeader><div className="space-y-4 py-4"><div className="space-y-2"><label className="text-sm font-medium">Название</label><Input value={newJournalTitle} disabled={mutationDisabled} onChange={e => setNewJournalTitle(e.target.value)} placeholder="Название" className="rounded-xl" /></div><div className="space-y-2"><label className="text-sm font-medium">Курс</label><Select value={selectedCourse} disabled={mutationDisabled} onValueChange={setSelectedCourse}><SelectTrigger className="rounded-xl"><SelectValue placeholder="Все ученики" /></SelectTrigger><SelectContent><SelectItem value="all" disabled={mutationDisabled}>Все ученики</SelectItem>{courses.map(c => <SelectItem key={c.id} value={c.id} disabled={mutationDisabled}>{c.title}</SelectItem>)}</SelectContent></Select></div></div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setShowCreateDialog(false)} className="rounded-xl">Отмена</Button><Button onClick={createJournal} disabled={mutationDisabled} className="rounded-xl">{saving && <SigmaSpinner size="sm" className="mr-2" />}Создать</Button></div></DialogContent>
      </Dialog>

      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Удалить журнал?</AlertDialogTitle><AlertDialogDescription>Вы уверены? Это действие нельзя отменить.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Отмена</AlertDialogCancel><AlertDialogAction onClick={deleteJournal} disabled={mutationDisabled} className="bg-destructive hover:bg-destructive/90">Удалить</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
      </AlertDialog>
    </div>
  );
}
