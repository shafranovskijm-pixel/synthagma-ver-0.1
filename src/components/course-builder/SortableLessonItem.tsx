...
function UploadProgressBlock({ label, progress, uploadStartTime, uploadedBytes, uploadFileSize, onCancel }: {
  label: string;
  progress: number;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-center gap-2"><SigmaSpinner size="sm" className="text-sigma-purple" /><span className="text-sm font-medium">{label}</span></div>
        <div className="w-full max-w-xs mx-auto">
          <div className="h-2 bg-secondary rounded-full overflow-hidden"><div className="h-full bg-sigma-purple transition-all duration-300 ease-out" style={{ width: `${progress}%` }} /></div>
        ...
        <Button variant="outline" size="sm" className="mt-2 gap-1 text-destructive hover:text-destructive border-destructive/50 hover:bg-destructive/10" onClick={onCancel}><Trash2 className="w-3 h-3" />Отменить</Button>
      </div>
    </div>
  );
}
