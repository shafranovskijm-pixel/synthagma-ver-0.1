...
export const ApiMonitorTab = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  (props, ref) => {
    ...
    const testFunction = useCallback(async (fnName: string): Promise<FunctionStatus> => {
      ...
      try {
        ...
        if (error) {
          // 4xx errors from the function itself still mean the function is reachable
          return {
            name: fnName,
...
ApiMonitorTab.displayName = "ApiMonitorTab";
