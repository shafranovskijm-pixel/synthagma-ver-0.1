...
export function CoursePreviewView({ courseId, embedded = false, onNavigateBack, onNavigateToEditor }: CoursePreviewViewProps) {
  ...
  if (!course) return (
    <div className={cn("flex items-center justify-center", embedded ? "py-20" : "min-h-screen bg-background")}>
      <div className="text-center"><BookOpen className="w-16 h-16 text-muted-foreground mx-auto mb-4" /><h1 className="text-2xl font-bold mb-4">Курс не найден</h1>{!embedded && <Button onClick={navigateBack}>Вернуться в панель</Button>}</div>
    </div>
  );

  return (
    <div className={cn("flex", embedded ? "rounded-2xl border border-border overflow-hidden bg-background min-h-[70vh]" : "min-h-screen bg-background")}>
      {/* Sidebar */}
      <aside className="w-80 bg-card border-r border-border flex flex-col">
        <div className="p-4 border-b border-border">
          ...
          <div className="mt-4">
            ...
            <Progress value={lessons.length ? (currentLessonIndex + 1) / lessons.length * 100 : 0} className="h-2" />
          </div>
        </div>
        <ScrollArea className="flex-1">
          <div className="p-2 space-y-1">
            {lessons.map((lesson, index) => {
              ...
              const isCurrent = index === currentLessonIndex && !showDocumentsView;
              return (
        ...
        <div className="p-4 border-t border-border space-y-3">
          ...
          {!embedded && !fromStore && <Button onClick={navigateToEditor} className="w-full gap-2" variant="outline"><Edit className="w-4 h-4" />Редактировать</Button>}
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="border-b border-border bg-card px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4 min-w-0">
            {!embedded && <><SigmaLogo size="sm" /><span className="text-muted-foreground">|</span></>}
            <Badge variant="outline" className="gap-1 text-sigma-cyan border-sigma-cyan/30 shrink-0"><Eye className="w-3 h-3" />Режим предпросмотра</Badge>
            <span className="font-medium truncate">{showDocumentsView ? 'Материалы курса' : currentLesson?.title}</span>
          </div>
        ...
        <ScrollArea className={cn("flex-1", embedded && "max-h-[calc(70vh-64px)]")} ref={contentRef}>
          <div className={cn("max-w-4xl mx-auto p-8 transition-all duration-300", isTransitioning ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0")}>
            ...
            <div className="flex items-center justify-between mt-12 pt-6 border-t border-border">
              ...
              : !embedded ? (fromStore ? <Button onClick={navigateBack} className="gap-2"><ArrowLeft className="w-4 h-4" />Назад в магазин</Button>
                : <Button onClick={navigateToEditor} className="gap-2"><Edit className="w-4 h-4" />Вернуться в редактор</Button>) : null}
            </div>
          </div>
        </ScrollArea>
      </main>
