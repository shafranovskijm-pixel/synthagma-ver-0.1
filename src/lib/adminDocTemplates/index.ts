...
function paidContract(v: AdminDocVariables): string {
  const body = `
    ...
    <p>1.1. Исполнитель обязуется оказать Заказчику услуги: <strong>${v.subject || "оказание образовательных услуг на платформе Синтагма"}</strong>, а Заказчик — принять и оплатить их.</p>
...
function freeContract(v: AdminDocVariables): string {
  const body = `
    ...
    <h2>1. Предмет договора</h2>
    <p>1.1. Исполнитель обязуется оказать Заказчику услуги <strong>на безвозмездной основе</strong>: ${v.subject || "предоставление доступа к образовательной платформе Синтагма"}.</p>
    <p>1.2. Договор заключён в целях, не связанных с извлечением прибыли (ст. 423 ГК РФ).</p>
    <h2>2. Срок</h2>
