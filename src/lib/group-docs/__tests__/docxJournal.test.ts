...
describe("generateClassJournalDocx", () => {
  ...
  it("looks up unknown operations read-only without treating unknown as completion", async () => {
    invokeMock.mockResolvedValue({ data: { compilerRevision: "goreltech-group-package-server-facts-v21", operationId: OP, operationStatus: "unknown", writesPerformed: false }, error: null });
  ...
  it.each(["operation", "revision", "status", "writes", "receipt-operation", "count", "hash", "batch", "type"])("rejects malformed status %s", async kind => {
    const payload = { compilerRevision: "goreltech-group-package-server-facts-v21", operationId: OP, operationStatus: "completed", writesPerformed: false,
  ...
  it("returns a confirmed nine-document receipt without claiming this lookup wrote", async () => {
    invokeMock.mockResolvedValue({ data: { compilerRevision: "goreltech-group-package-server-facts-v21", operationId: OP, operationStatus: "completed", writesPerformed: false,
  ...
  it.each([
    ...
    invokeMock.mockResolvedValue({ data: {
      compilerRevision: "goreltech-group-package-server-facts-v21", operationId: OP,
  ...
  beforeEach(() => {
    ...
    supabaseInvokeMock.mockResolvedValue({
      ...
      error: {
        context: {
          headers: new Headers({
            "X-Sintagma-Compiler-Revision": "goreltech-group-package-server-facts-v21",
          }),
        },
      },
    });
  ...
  it("возвращает данные атомарной партии", async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith("compile-group-class-journal", {
      ...
      headers: { "X-Sintagma-Required-Compiler-Revision": "goreltech-group-package-server-facts-v21" },
    });
  });
  ...
  it("проверяет полный пакет на сервере без Storage/DB-записи", async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith("compile-group-class-journal", {
      ...
      headers: {
        "X-Sintagma-Required-Compiler-Revision": "goreltech-group-package-server-facts-v21",
      },
    });
    expect(supabaseInvokeMock).toHaveBeenCalledWith("compile-group-class-journal", {
      body: { capabilityProbe: true },
  ...
  ])("не отправляет данные группы при неподтверждённой для v21 ревизии %s", async (revision) => {
    supabaseInvokeMock.mockResolvedValue({
      data: null,
      error: {
        context: {
  ...
  it("подтверждает v21 по JSON, когда Nginx не exposes response header", async () => {
  ...
  it.each([
    ...
    ["с неожиданно успешным probe", {
      data: { compilerRevision: "goreltech-group-package-server-facts-v21" },
      error: null,
    }],
  ...
  it.each([undefined, null, 16, "goreltech-group-package-server-facts-v15", "goreltech-group-package-server-facts-v19", "goreltech-group-package-server-facts-v20"])(
  ...
  it("не принимает dry-run от другой версии после успешного v21 preflight", async () => {
    invokeMock.mockResolvedValue({
      data: {
