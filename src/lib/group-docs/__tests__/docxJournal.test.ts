...
describe("generateClassJournalDocx", () => {
  beforeEach(() => {
    ...
    supabaseInvokeMock.mockResolvedValue({
      ...
      error: {
        context: {
          headers: new Headers({
            "X-Sintagma-Compiler-Revision": "goreltech-group-package-server-facts-v17",
          }),
        },
      },
    });
  ...
  it("возвращает данные атомарной партии", async () => {
    invokeMock.mockResolvedValue({
      data: {
        compilerRevision: "goreltech-group-package-server-facts-v17",
    ...
    expect(invokeMock).toHaveBeenCalledWith("compile-group-class-journal", {
      ...
      headers: { "X-Sintagma-Required-Compiler-Revision": "goreltech-group-package-server-facts-v17" },
    });
  });
  ...
  it("проверяет полный пакет на сервере без Storage/DB-записи", async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith("compile-group-class-journal", {
      ...
      headers: {
        "X-Sintagma-Required-Compiler-Revision": "goreltech-group-package-server-facts-v17",
      },
    });
    expect(supabaseInvokeMock).toHaveBeenCalledWith("compile-group-class-journal", {
      body: { capabilityProbe: true },
  ...
  ])("не отправляет данные группы при неподтверждённой для v17 ревизии %s", async (revision) => {
    supabaseInvokeMock.mockResolvedValue({
      data: null,
      error: {
        context: {
  ...
  it("подтверждает v17 по JSON, когда Nginx не exposes response header", async () => {
  ...
  it.each([
    ...
    ["с неожиданно успешным probe", {
      data: { compilerRevision: "goreltech-group-package-server-facts-v17" },
      error: null,
    }],
  ...
  it.each([undefined, null, 16, "goreltech-group-package-server-facts-v15", "goreltech-group-package-server-facts-v18"])(
  ...
  it("не принимает dry-run от другой версии после успешного v17 preflight", async () => {
    invokeMock.mockResolvedValue({
      data: {
