...
const features = [
  ...
  { img: featFrdo, title: "ФИС ФРДО", text: "Проверка данных и подготовка XLSX для последующей загрузки ответственным сотрудником организации." },
  ...
  { img: featCrm, title: "CRM и продажи (Beta)", text: "Пилотный модуль показываем только после отдельной проверки нужного сценария." },
];

const proposalHighlights = [
  "Состав платформы и модули по разделам",
  "Тарифы и лимиты с актуальными ценами",
  "ФИС ФРДО: проверка данных и подготовка XLSX; загрузку выполняет ответственный сотрудник организации",
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Живая демонстрация СИНТАГМЫ: конструктор курсов, ученики, документы и подготовка XLSX для ФИС ФРДО." />
      ...
      <section id="features" className="py-16 md:py-24 bg-secondary/20">
        <div className="container mx-auto px-6">
          <div className="max-w-3xl mx-auto text-center mb-12">
            ...
            <p className="text-muted-foreground">
              Основные процессы СИНТАГМЫ — от конструктора курсов до подготовки данных и XLSX для ФИС ФРДО. Beta-модули показываются отдельно.
            </p>
          </div>
